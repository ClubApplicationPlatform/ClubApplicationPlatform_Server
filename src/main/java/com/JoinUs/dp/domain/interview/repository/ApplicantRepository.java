...
public interface ApplicantRepository extends JpaRepository<Applicant, Long> {
    ...
    // 면접 중복 신청 방지에 사용됨
    boolean existsByUser_EmailAndInterview_Club_Id(String email, Long clubId);

    // 특정 면접 회차에 지원한 모든 유저(신청자) 목록
    // 면접 현황 조회 기능에 사용됨
    List<Applicant> findAllByInterview_Id(Long interviewId);

    // 특정 동아리에 합격한 유저(신청자)들의 목록. 동아리Id, 합격여부, 확정여부 로 필터링 됨
    // 미확정/가입거절 인원 조회에 사용됨
    List<Applicant> findAllByInterview_Club_IdAndInterviewResultAndConfirmStatus(
            Long clubId,
    ...
    Long countByInterview_Club_Id(Long clubId);

    // 미확정/가입거절, 거절 인원수 조회
    Long countByInterview_Club_IdAndInterviewResultAndConfirmStatus(
            Long clubId,
            InterviewResultEnum interviewResult,
            InterviewConfirmedEnum confirmStatus
    );
